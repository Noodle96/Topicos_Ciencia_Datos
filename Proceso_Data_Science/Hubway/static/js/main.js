...
async function initializeApp() {
    ...
    const spatialResponse = await fetch("/api/stations-spatial");
    spatialStationsData = await spatialResponse.json();
...
function renderMunicipalMap(svgSelector, stationsData, selectedMunicipal) {
    const { chartGroup, innerWidth, innerHeight } = prepareSvg(svgSelector);
    ...
    const xExtent = d3.extent(validStations, (d) => d.lng);
    const yExtent = d3.extent(validStations, (d) => d.lat);

    const xScale = d3
        .scaleLinear()
        .domain(xExtent)
        .range([40, innerWidth - 40]);
    ...
    const selectedStations = validStations.filter(
        (d) => d.municipal === selectedMunicipal
    );

    const otherStations = validStations.filter(
        (d) => d.municipal !== selectedMunicipal
    );

    // Dibujar estaciones de otras municipalidades
    chartGroup
        .selectAll(".other-station")
        .data(otherStations)
        .enter()
        .append("circle")
    ...
    // Dibujar estaciones de la municipalidad seleccionada
    chartGroup
        .selectAll(".selected-station")
    ...
    // Calcular centro referencial
    const centerLat = d3.mean(selectedStations, (d) => d.lat);
    const centerLng = d3.mean(selectedStations, (d) => d.lng);

    if (centerLat !== undefined && centerLng !== undefined) {
        // Halo exterior
        chartGroup
            .append("circle")
            .attr("cx", xScale(centerLng))
            .attr("cy", yScale(centerLat))
            .attr("r", 12)
            .attr("fill", "#4A90C2")
            .attr("opacity", 0.18);
        ...
        chartGroup
            .append("text")
            .attr("x", xScale(centerLng) + 10)
            .attr("y", yScale(centerLat) - 8)
            .style("font-size", "12px")
            .style("font-weight", "bold")
            .text("Centro referencial");
    }
    ...
    legendGroup
        ...
        .attr("y", 4)
        .style("font-size", "12px")
        .text(`Estaciones de ${selectedMunicipal}`);
