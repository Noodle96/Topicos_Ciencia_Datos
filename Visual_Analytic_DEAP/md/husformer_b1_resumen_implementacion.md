...
**⚠️ Actualización (2026-07-22, a pedido de Russell):** el B2 ORIGINAL (líneas superpuestas) se descarta del sistema por completo -- B1 ya NO comparte panel con un selector Heatmap/Líneas, siempre muestra el heatmap. B1 vuelve a ocupar 1/3 de Vista B (antes era un panel más ancho, 1.1fr de 3.6fr); el panel de señal cruda -- reetiquetado de B3 a B2 el mismo día, ver `husformer_b3_resumen_implementacion.md` -- ocupa los otros 2/3. Las secciones 2.6/2.7 de abajo (fusión con el B2 original, resaltado sincronizado con "B3") describen el estado ANTERIOR -- se conservan como registro histórico de esa decisión de diseño, ya no reflejan el código actual (donde el panel de señal cruda ahora se llama B2). `husformer_b2_chart.js` no se borró (código muerto, disponible si se reconsidera). Ver `husformer_a3_resumen_implementacion.md` Sección 4 para el detalle completo del reajuste de layout.
