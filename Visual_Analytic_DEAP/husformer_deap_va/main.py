...
# FIX (2026-07-04, husformer_deap_va): la clave original era 'Husformer' (con
# mayúscula), pero `dataset` ya se pasó por str.lower() arriba -> el .get()
# nunca encontraba la clave y siempre caía al valor por defecto (1). Además,
# el valor correcto para nuestro esquema de 3 clases (labeling.py + el fix de
# remake_label en src/utils.py) es 3, no 1: focalloss hace
# log_softmax(pred, dim=1).gather(..., index=target) con target en {0,1,2},
# así que la última capa del modelo (out_layer) debe producir 3 valores por
# ventana, no 1 (con output_dim=1 el gather lanza index out of range en
# cuanto aparece una ventana de valencia alta, etiqueta 2).
output_dim_dict = {
    'husformer': 3,
}
