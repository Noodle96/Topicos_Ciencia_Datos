...
def get_channels_from_raw(
    ...
    # # entonces raw.n_times es el número de muestras = numero_de_registros_X_ frecuencia_muestreo
    ...
    try:
        ...
        # print("signals_1 shape:", signals_1.shape) # signals_1 shape: (20, 362250) # 20 definido en el montage_list_1 y montage_list_2
        # print("signals_1:", signals_1)
        signals_2: np.ndarray = raw.get_data(picks=montage_indices_2)
    except Exception:
        print("[Exception] Something is wrong when reading channels of the raw EEG signal")
...
def butter_bandpass(
    ...
    Este filtro atenúa las frecuencias fuera del rango [lowcut, highcut]
    y preserva las componentes dentro de ese rango.
    ...
    Returns:
        Tuple[np.ndarray, np.ndarray]: Coeficientes del filtro (b, a) que
        pueden ser usados por funciones como `filtfilt` para aplicar el filtro.
    """
    nyq: float = 0.5 * fs
...
def butter_bandpass_filter(
    ...
    Utiliza los coeficientes generados por la función `butter_bandpass`
    y filtra la señal para retener solamente las frecuencias en el
    rango [lowcut, highcut].
...
def slice_signals_into_binary_segments(
    ...
    labels: List[Tuple[int, int, str]],     # Intervalos con clase: [(inicio, fin, etiqueta)]
    segment_interval: float,                # Duración del segmento en segundos => ej. 4.0 seg
    seizure_types: List[str],               # ['bckg', 'seizure']
    seizure_overlapping_ratio: List[float]  # [0.0, 0.75] → cuánto se solapan las ventanas por clase
) -> List[List[List[np.ndarray]]]:
    """
    Segmenta señales EEG en ventanas fijas y las clasifica como fondo ("bckg") o crisis ("seizure").

    Esta función recorre los intervalos anotados (por ejemplo, de tipo 'bckg' o 'seizure') y
    divide cada intervalo en múltiples ventanas deslizantes (sliding windows) de longitud fija.
    Aplica verificación de ruido y longitud, y organiza los segmentos válidos según su tipo.
    ...
    La estructura del retorno es jerárquica de 3 niveles:
        ...
        │
        └── Nivel 3: para cada intervalo, una lista de ventanas válidas (una por segmento temporal)
    ...
    Args:
        ...
        labels (List[Tuple[int, int, str]]): Lista de etiquetas con intervalos en segundos y clase asociada.
                                             Ejemplo: [(12, 25, 'bckg'), (25, 40, 'seizure')].
        segment_interval (float): Duración de cada ventana en segundos (ej. 4.0).
        seizure_types (List[str]): Lista de clases binarias. Usualmente: ['bckg', 'seizure'].
        seizure_overlapping_ratio (List[float]): Lista con el grado de solapamiento (entre 0 y 1)
                                                 para cada tipo de clase. Ejemplo: [0.0, 0.75].

    Returns:
        List[List[List[np.ndarray]]]: Segmentos EEG organizados por clase → intervalo → ventanas válidas.
    """
...
def resample_data_in_each_channel(
    ...
    individualmente a cada una, adaptando su número de muestras según la
    nueva frecuencia de muestreo.

    Args:
        signals (List[np.ndarray]): Lista de señales (una por canal), cada una como array 1D.
        thisFS (int): Frecuencia de muestreo original (Hz).
        resampleFS (int): Nueva frecuencia de muestreo deseada (Hz).

    Returns:
        List[np.ndarray]: Lista de señales remuestreadas, en el mismo orden que la entrada.
    """
...
def summarize_raw(raw: mne.io.BaseRaw) -> Dict[str, Any]:
    ...
    # mostrar la frecuencia de cada canal y mostrar con su nombre del canal
    ...
    for idx in range(min(5, n_channels)):
        print(f"  {chan_names[idx]:<10} – µ={means[idx]:.9f}, σ={stds[idx]:.9f}")
...
def get_channel_frequencies_from_edf(edf_path: str) -> List[Tuple[str, float]]:
    ...
    Returns:
        List[Tuple[str, float]]: Lista de tuplas con el nombre del canal y su frecuencia de muestreo.
    """
...
def get_labels_complete_from_csv_bi_clasificacion_binaria(data_path: str) -> List[Tuple[int, int, str]]:
    """
